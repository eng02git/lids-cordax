		...
		with st.form('adicionar ferramental'):
			...
			if submitted:
				...
				if not df_ferramenta[df_ferramenta['ID'] == documento].shape[0] > 0:
					...
					# Armazena no banco
					arquivo = colecao + '.csv'
					df_ferramenta.to_csv(arquivo, index=False)
...
def teste(val_max, val_min, titulo, medida, colecao, dados, conjunto):
	...
	with st.beta_expander('Histórico Strokes por ferramenta'):
		...
		with col2:
			...
			#col2.write(fig)
			fig = px.histogram(df_plot2, x="Total de Strokes da ferramenta", nbins=20, height=600, width=1200)
...
with st.beta_expander('Teste de grafana', expanded=True):
	components.iframe("http://192.168.81.128:3000/grafana/d/8Kq5oDMnk/teste-copy?orgId=1&refresh=1m&kiosk", height=650)
	#components.iframe("http://10.186.96.18:3000/grafana/d/-ZYXo_qMk/status?orgId&kiosk", height=650)
	
try:
	#client2 = InfluxDBClient(host='10.186.96.18', port=8086, database='grafana_usage')
...
#q2 = 'SELECT * FROM (SELECT "Segundos" as "valor" FROM "L2_FELC" WHERE time > now() - 1h);'


lista1 = pd.DataFrame(client.query(q1).get_points())
#lista2 = pd.DataFrame(client2.query(q2).get_points())

#st.write(lista2.iloc[0,:])

num_strokes = int(lista1.sort_values(by='time', ascending=False).iloc[0,1])	
